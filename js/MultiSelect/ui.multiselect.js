...
//    mouseX = e.pageX; 
//    mouseY = e.pageY;
//    $('.tooltipbox').css('top', mouseY + 5);
//    $('.tooltipbox').css('left', mouseX + 15);
//});
...
(function($) {
    ...
    $.widget("ui.multiselect", {
        ...
        _create: function() {
            ...
            this.tooltipbox = $('<div class="tooltipbox ui-widget-shadow" style="display:none"></div>').appendTo(this.container);
            ...
            //Set up css for tooltip box
            $(this.tooltipbox).css('zIndex', 9999);
            $(this.tooltipbox).css('position','absolute');
            $(this.tooltipbox).css('max-width','250px');
            $(this.tooltipbox).css('font-size','10px');
        },
    ...
    **************************************/
        ...
        _insertToList: function(node, list) {
            //hacky way of adding a class to every node!
            node.addClass('multiselect_node')
        ...
        _updateCount: function() {
            ...
            if (this.busy) setTimeout(function() {
                that._updateCount();
            }, 100);
        ...
        _getOptionNode: function(option) {
            ...
            var tags = option.attr('tags');
            if(tags != undefined){
                tags = option.attr('tags').replace(/"/g, '&quot;');
            } else {
                tags = '';
            }
            
            var info = option.attr('info');
            if(info != undefined){
                info = option.attr('info').replace(/"/g, '&quot;');
            } else {
                info = 'No Info';
            }
            
            var node = $('<li '+'tags="'+ tags +'"'+' class="ui-state-default ui-element ' + option.attr('class') + '" info="'+info+'"><span class="ui-icon"/>'+option.text()+' <span class="trashbin ui-icon ui-icon-trash"/><a href="#" class="ui-state-default action"><span class="ui-corner-all ui-icon"/></a></li>').hide();
            ...
            if ( !term ) {
                ...
                elements.each(function(i,element) {
                    ...
                    for(var index in terms) {
                        var aTerm = terms[index];
                      
                        if((element.text() + ' ' +  element.attr('tags')).toLowerCase().indexOf(aTerm) < 0) {
                            hasAll = false;
                            break;
                        }
                    }
                    element[(hasAll ? 'remove' : 'add')+'Class']('filtered')
                });
            }
        ...
        _registerHoverEvents: function(elements) {
            ...
            .unbind('mouseover.multiselect').bind('mouseover.multiselect', function() {
                ...
                var thisElementHere = $(this).find('a').andSelf();
                $(that.tooltipbox).html( thisElementHere.attr('info'));
                $(that.tooltipbox).show();
            })
            .unbind('mouseout.multiselect').bind('mouseout.multiselect', function() {
                $(this).find('a').andSelf().removeClass('ui-state-hover');
                $(that.tooltipbox).hide();

            }).unbind('mousemove.multiselect').bind('mousemove.multiselect', function(e) {
                var off = $(that.container).offsetParent();
                
                var top = 0;
                var left = 0;
                
                while(!$('body').is(off)) {
                    top += off.position().top;
                    left += off.position().left;
                    off = off.offsetParent();
                    
                }


                $(that.tooltipbox).css('top', e.pageY - top + 10);
                $(that.tooltipbox).css('left', e.pageX - left + 10);
